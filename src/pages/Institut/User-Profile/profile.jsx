...
import {
  ...
  Button, Upload, Divider, Statistic, Form, Input, Modal, Select, DatePicker
} from "antd";
import {
                ...
                >
                  ...
                  {!isEditing && (
                    <Descriptions column={1} size="middle">
                      ...
                      <Descriptions.Item label={<Space><UserOutlined /> <span>{t("profilePage.fields.fullName")}</span></Space>}>
                        <Text>{userData.firstName || ""} {userData.lastName || ""}</Text>
                      </Descriptions.Item>
                      <Descriptions.Item label={<Space><PhoneOutlined /> <span>{t("profilePage.fields.phone")}</span></Space>}>
                        <Text copyable>{userData.phone || t("profilePage.gender.UNSPECIFIED")}</Text>
                      </Descriptions.Item>
                      <Descriptions.Item label={<Space><HomeOutlined /> <span>{t("profilePage.fields.adress")}</span></Space>}>
                        <Text>{userData.adress || t("profilePage.gender.UNSPECIFIED")}</Text>
                      </Descriptions.Item>
                      <Descriptions.Item label={<Space><GlobalOutlined /> <span>{t("profilePage.fields.placeOfBirth")}</span></Space>}>
                        <Text>{userData.placeOfBirth || t("profilePage.gender.UNSPECIFIED")}</Text>
                      </Descriptions.Item>
                      <Descriptions.Item label={<Space><CalendarOutlined /> <span>{t("profilePage.fields.dateOfBirth")}</span></Space>}>
                        <Text>{formatDate(userData.dateOfBirth)}</Text>
                      </Descriptions.Item>
                      <Descriptions.Item label={<Space><GlobalOutlined /> <span>{t("profilePage.fields.country")}</span></Space>}>
                        <Text>{userData.country || t("profilePage.gender.UNSPECIFIED")}</Text>
                      </Descriptions.Item>
                      <Descriptions.Item label={<Space><IoTransgender /> <span>{t("profilePage.fields.gender")}</span></Space>}>
                        <Text>{getGenderText(userData.gender)}</Text>
                      </Descriptions.Item>
                      <Descriptions.Item label={<Space><CalendarOutlined /> <span>{t("profilePage.fields.createdAt")}</span></Space>}>
                        {formatDate(userData.createdAt)}
                      </Descriptions.Item>
                      <Descriptions.Item label={<Space><ClockCircleOutlined /> <span>{t("profilePage.fields.updatedAt")}</span></Space>}>
                ...
                <Col xs={24} lg={12}>
                  <Card
                    title={
                      <Space>
                        <BankOutlined />
                        <span>{t("profilePage.orgEditor.title")}</span>
                      </Space>
                    }
                    extra={
                      !isEditingOrg && canEditOrg && (
                        <Button type="link" icon={<EditOutlined />} onClick={handleEditOrg}>
                  ...
                  >
                    <Form form={orgForm} layout="vertical" onFinish={handleUpdateOrg} style={{ display: (isEditingOrg && canEditOrg) ? "block" : "none" }}>
                        <Form.Item
                          ...
                          rules={[{ required: true, message: t("profilePage.orgEditor.validators.nameRequired") }]}
                        >
                        ...
                        <Form.Item
                          name="type"
                          label={t("profilePage.orgEditor.type")}
                          rules={[{ required: true, message: t("profilePage.orgEditor.validators.typeRequired") }]}
                        >
                          <Select placeholder={t("profilePage.orgEditor.selectType")} options={ORG_TYPES} />
                        </Form.Item>

                        <Form.Item name="email" label={t("profilePage.orgEditor.email")}>
                          <Input placeholder={t("profilePage.orgEditor.email")} type="email" />
                        </Form.Item>
                        ...
                        <Form.Item>
                          <Space>
                            <Button
                              type="primary"
                              loading={loading}
                              icon={!loading ? <PlusCircleOutlined /> : undefined}
                              htmlType="submit"
                            >
                              {t("profilePage.buttons.save")}
                            </Button>
